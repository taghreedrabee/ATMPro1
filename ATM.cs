...
namespace ATMapp
{
    internal class ATM
    {
            ...
            private static List<PendingTransfer> _pendingTransfers = new List<PendingTransfer>();

            public ATM(IUserRepository userRepository, ITransactionRepository transactionRepository)
            {
            ...
            private void DeleteUser()
            {
                ...
                var sentPendingTransfers = _pendingTransfers.Where(pt => pt.SenderId == userToDelete.UserId).ToList();
                var receivedPendingTransfers = _pendingTransfers.Where(pt => pt.RecipientId == userToDelete.UserId).ToList();
                ...
                if (hasTransactions || hasPendingTransfers)
                {
                    ...
                    if (_userRepository.DeleteUser(username, password))
                    {
                        ...
                        _transactionRepository.AddTransaction(new TransactionInfo(
                            Guid.NewGuid(), Guid.Empty, "System", "User Deletion",
                            0, DateTime.Now, 0, 0, null, true
                        ));
                    }
                    else
                    {
            ...
            private void CreateNewUser()
            {
                ...
                IUser newUser = new User(Guid.NewGuid(), username, password, email, birthDate, userType == "VIP" ? UserType.VIP : UserType.Ordinary);
            ...
            {
                var userPendingTransfers = _pendingTransfers.Where(pt => pt.RecipientId == _currentUser.UserId).ToList();
                ...
                for (int i = 0; i < userPendingTransfers.Count; i++)
                {
                    ...
                    Console.WriteLine($"{i + 1}- From: {transfer.SenderUsername}, Amount: {transfer.Amount:C}, Date: {transfer.DateTime}");
                }
            ...
            { 
                var returnedTransfers = _transactionRepository.GetFinancialTransactions(_currentUser.UserId)
                    .Where(t => t.TransactionName == "Transfer Cancelled - User Deletion" ||
                                t.TransactionName == "Transfer Returned - Recipient Deleted" ||
                                t.TransactionName == "Transfer (Refunded)" ||
                                (t.TransactionName == "Transfer (Sent - Pending)" &&
                                 !_pendingTransfers.Any(pt => pt.SenderId == _currentUser.UserId &&
                                                             pt.Amount == t.Amount &&
                ...
                var pendingSentTransfers = _pendingTransfers
                    .Where(pt => pt.SenderId == _currentUser.UserId)
                    .ToList();

                double totalPendingMoney = pendingSentTransfers.Sum(pt => pt.Amount);
                ...
                Console.WriteLine("\n2. Pending Transfer Information:");
                if (pendingSentTransfers.Any())
                {
                    Console.WriteLine($"   Money in pending transfers (waiting for approval): {totalPendingMoney:C}");
                    foreach (var transfer in pendingSentTransfers)
                    {
                    
                        var recipient = _userRepository.GetUserById(transfer.RecipientId);
                        string recipientName = recipient?.Username ?? "Unknown User";
                        Console.WriteLine($"   - {transfer.Amount:C} pending approval from {recipientName}");
                    }
                    Console.WriteLine($"   Available balance after all pending transfers: {_currentUser.Balance + totalPendingMoney:C}");
                }
                else
                {
                    Console.WriteLine("   No pending transfers");
            ...
            private void TransferMoney()
            {
                ...
                var recipientPendingTransfers = _pendingTransfers
                    .Where(pt => pt.RecipientId == recipient.UserId)
                    .ToList();

                if (recipientPendingTransfers.Any())
                {
                    Console.WriteLine($"Warning: Recipient has {recipientPendingTransfers.Count} pending transfers.");
                    Console.Write("Do you want to continue with the transfer? (y/n): ");
